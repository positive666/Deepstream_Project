...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    std::map<std::string, Weights> weightMap = loadWeights("../arcface-100mxnet.wts");
    Weights emptywts{DataType::kFLOAT, nullptr, 0};
...
void doInference(IExecutionContext& context, float* input, float* output, int batchSize,bool onnx_flag) {
...
int main(int argc, char** argv) {
	...
	else if (argc == 2 && std::string(argv[1]) == "-o") {
		std::fstream existEngine;
		nvinfer1::ICudaEngine *engine_onnx;
		existEngine.open("./arcface-r100.trt", std::ios::in);
	    onnxToTRTModel("../arcface_r100.onnx", "./arcface-r100.trt", engine_onnx, BATCH_SIZE);
        assert(engine_onnx != nullptr);
	...
	else if (argc == 2 && std::string(argv[1]) == "-onnx") {
